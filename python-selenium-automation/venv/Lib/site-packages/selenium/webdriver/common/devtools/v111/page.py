...
class Frame:
    '''
    Information about the Frame on the pages.
    '''
...
class FrameResource:
    '''
    Information about the Resource on the pages.
    '''
    #: Resource URL.
    url: str
...
def bring_to_front() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,None]:
    '''
    Brings pages to front (activates tab).
    '''
    cmd_dict: T_JSON_DICT = {
...
def capture_snapshot(
    ...
    '''
    Returns a snapshot of the pages as a string. For MHTML format, the serialization includes
    iframes, shadow DOM, external resources, and element-inline styles.

    **EXPERIMENTAL**

    :param format_: *(Optional)* Format (defaults to mhtml).
    :returns: Serialized pages data.
    '''
    params: T_JSON_DICT = dict()
    if format_ is not None:
        params['format'] = format_
...
def disable() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,None]:
    '''
    Disables pages domain notifications.
    '''
    cmd_dict: T_JSON_DICT = {
        'method': 'Page.disable',
    }
...
def enable() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,None]:
    '''
    Enables pages domain notifications.
    '''
    cmd_dict: T_JSON_DICT = {
        'method': 'Page.enable',
    }
...
def get_cookies() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,typing.List[network.Cookie]]:
    '''
    Returns all browser cookies for the pages and all of its subframes. Depending
...
def get_layout_metrics() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,typing.Tuple[LayoutViewport, VisualViewport, dom.Rect, LayoutViewport, VisualViewport, dom.Rect]]:
    '''
    Returns metrics relating to the layouting of the pages, such as viewport bounds/scale.
...
def get_navigation_history() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,typing.Tuple[int, typing.List[NavigationEntry]]]:
    '''
    Returns navigation history for the current pages.
...
def reset_navigation_history() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,None]:
    '''
    Resets navigation history for the current pages.
    '''
    cmd_dict: T_JSON_DICT = {
...
def navigate(
    ...
    '''
    Navigates current pages to the given URL.

    :param url: URL to navigate the pages to.
...
def navigate_to_history_entry(
    ...
    '''
    Navigates current pages to the given history entry.
...
def print_to_pdf(
    ...
    :param page_ranges: *(Optional)* Paper ranges to print, one based, e.g., '1-5, 8, 11-13'. Pages are printed in the document order, not in the order specified, and no more than once. Defaults to empty string, which implies the entire document is printed. The pages numbers are quietly capped to actual pages count of the document, and ranges beyond the end of the document are ignored. If this results in no pages to print, an error is reported. It is an error to specify a range with start greater than end.
    :param header_template: *(Optional)* HTML template for the print header. Should be valid HTML markup with following classes used to inject printing values into them: - ```date````: formatted print date - ````title````: document title - ````url````: document location - ````pageNumber````: current pages number - ````totalPages````: total pages in the document  For example, ````<span class=title></span>```` would generate span containing the title.
    :param footer_template: *(Optional)* HTML template for the print footer. Should use the same format as the ````headerTemplate````.
    :param prefer_css_page_size: *(Optional)* Whether or not to prefer pages size as defined by css. Defaults to false, in which case the content will be scaled to fit the paper size.
...
def reload(
    ...
    '''
    Reloads given pages optionally ignoring the cache.

    :param ignore_cache: *(Optional)* If true, browser cache is ignored (as if the user pressed Shift+refresh).
    :param script_to_evaluate_on_load: *(Optional)* If set, the script will be injected into all frames of the inspected pages after reload. Argument will be ignored if reloading dataURL origin.
    '''
    params: T_JSON_DICT = dict()
...
def set_bypass_csp(
    ...
    '''
    Enable pages Content Security Policy by-passing.

    **EXPERIMENTAL**

    :param enabled: Whether to bypass pages CSP.
    '''
    params: T_JSON_DICT = dict()
    params['enabled'] = enabled
    cmd_dict: T_JSON_DICT = {
...
def set_lifecycle_events_enabled(
    ...
    '''
    Controls whether pages will emit lifecycle events.
...
def stop_loading() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,None]:
    '''
    Force the pages stop all navigations and pending resource fetches.
    '''
    cmd_dict: T_JSON_DICT = {
...
def close() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,None]:
    '''
    Tries to close pages, running its beforeunload hooks, if any.
...
def set_web_lifecycle_state(
    ...
    '''
    Tries to update the web lifecycle state of the pages.
    It will transition the pages to the given state according to:
...
def produce_compilation_cache(
    ...
    would be produced. The list may be reset during pages navigation.
...
def wait_for_debugger() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,None]:
    '''
    Pauses pages execution. Can be resumed using generic Runtime.runIfWaitingForDebugger.
...
class FileChooserOpened:
    '''
    Emitted only when ``pages.interceptFileChooser`` is enabled.
    '''
...
class DownloadWillBegin:
    ...
    **EXPERIMENTAL**
    ...
    Fired when pages is about to start a download.
...
class InterstitialHidden:
    '''
    Fired when interstitial pages was hidden
    '''
...
class InterstitialShown:
    '''
    Fired when interstitial pages was shown
    '''
...
class JavascriptDialogOpening:
    ...
    #: the pages execution. Execution can be resumed via calling Page.handleJavaScriptDialog.
...
class LifecycleEvent:
    '''
    Fired for top level pages lifecycle events such as navigation, load, paint, etc.
    '''
    #: Id of the frame.
    frame_id: FrameId
...
class BackForwardCacheNotUsed:
    ...
    **EXPERIMENTAL**
    ...
    frame_id: FrameId
    #: Array of reasons why the pages could not be cached. This must not be empty.
    not_restored_explanations: typing.List[BackForwardCacheNotRestoredExplanation]
    #: Tree structure of reasons why the pages could not be cached for each frame.
...
class ScreencastVisibilityChanged:
    ...
    **EXPERIMENTAL**
    ...
    Fired when the pages with currently enabled screencast was shown or hidden .
    '''
    #: True if the pages is visible.
    visible: bool
